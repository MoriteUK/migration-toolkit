...
function registerIPCHandlers() {
  ipcMain.handle('launch-script', async (event, scriptName, args = []) => {
    ...
    try {
      ...
      const targetArgs = Array.isArray(args) ? args.map(String) : [];
      const innerList = ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', scriptPath, ...targetArgs]
        .map(sq).join(', ');

      const wrapperBody = [
        `# Auto-generated by Migration Toolkit launch-script - safe to delete.`,
        `try { $Host.UI.RawUI.WindowTitle = ${sq(scriptName + ' - Migration Toolkit')} } catch {}`,
        `Set-Location ${sq(PS_SCRIPT_PATH)}`,
        `$pwsh = (Get-Command pwsh.exe -ErrorAction SilentlyContinue).Source`,
        `if (-not $pwsh) { $pwsh = 'pwsh.exe' }`,
        `& $pwsh @(${innerList})`,
        `$code = $LASTEXITCODE`,
        `Write-Host ''`,
        `Write-Host ('=== ' + ${sq(scriptName)} + ' finished (exit ' + $code + '). ===') -ForegroundColor Yellow`,
      ...
      const wrapperPath = path.join(
        os.tmpdir(),
        `mtk-launch-${scriptName.replace(/[^\w.-]+/g, '_')}-${Date.now()}.ps1`
      );
